 ...
 *
 * Description: If file is not ELF file the exit code is 98.
 */
void check_elf(unsigned char *e_ident)
{
	int index;
 ...
 * @e_ident: A pointer to array containing ELF magic numbers.
 *
 * Description: Magic numbers are separated by a spaces.
 */
void print_magic(unsigned char *e_ident)
{
	int index;
 ...
 *
 * Description: If file cannot be closed - exit code is 98.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
 ...
 *
 * Description: If the file is not an ELF File or the function fails - exit code is 98.
 */
